// AUTO-GENERATED — candidate request headers for the unkeyed-header (cache-poisoning) scan.
// Source: PortSwigger Param Miner resources/headers, plus a curated high-value set ordered
// first (so an early stop still covers the best vectors). Request-framing headers
// (host, content-length, transfer-encoding, connection, …) are excluded — injecting them
// corrupts the probe rather than testing the cache. Header names are case-insensitive.
// Regenerate by re-running the build step in the commit that introduced this file.
export const POISON_HEADERS: string[] = [
  "x-forwarded-host",
  "x-host",
  "x-original-host",
  "x-forwarded-server",
